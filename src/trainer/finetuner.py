...
from src.utils import get_memory_usage, get_system_info, load_checkpoint


class FineTuner:
    """FineTuner for applying LoRA to a pretrained GPT model.

    Handles loading a base model checkpoint, applying LoRA adapters,
    and fine-tuning on a specified dataset with gradient accumulation
    ...
    def __init__(self, checkpoint_path, finetune_config: FinetuneConfig, gpt_config: GPTConfig, device: str):
        ...
        lora_config = LoraConfig(
            r=8, lora_alpha=16, lora_dropout=0.1, bias="none", target_modules=["c_attention", "c_projection"]
        )
    ...
    def save_checkpoint(
        self, iter_num: int, loss: float, name: str = "ckpt_finetune.pt", cleanup_after_logging: bool = False
    ):
        """Save LoRA adapter checkpoint and optionally log to MLflow."""
        ...
        # Log to MLflow
        print("Logging checkpoint and model to MLflow...")
        try:
            ...
            print("Checkpoint and model logged to MLflow")
    ...
    def finetune_step(self):
        """Perform a single finetuning step with gradient accumulation."""
        t0 = time.time()  # Start timer
        for _micro_step in range(self.finetune_config.gradient_accumulation_steps):
            ...
            with torch.amp.autocast(device_type="cuda", enabled=(self.device == "cuda")):
                ...
                loss = (
                    loss / self.finetune_config.gradient_accumulation_steps
    ...
    def finetune(self):
        """Execute the complete finetuning process for the specified number of epochs."""
        start_finetune_time = time.time()
        print(
            f"Starting finetuning for {self.finetune_config.num_epochs} epochs... "
        ...
        try:
            for epoch in range(self.finetune_config.num_epochs):
                ...
                for _local_iter_num in range(
                    self.dataloader.get_data_size("finetune")
                    % (self.finetune_config.batch_size * self.finetune_config.gradient_accumulation_steps)
                ):
                    ...
                    tokens_per_sec = (
                        self.finetune_config.batch_size
                        * self.gpt_config.max_position_embeddings
                        * self.finetune_config.gradient_accumulation_steps
                    ) / duration
                ...
                average_epoch_loss = total_epoch_loss / (_local_iter_num + 1)
                ...
                if (epoch + 1) % self.finetune_config.checkpoint_interval == 0:
                    self.save_checkpoint(
                        self.iter_num,
                        average_epoch_loss,
                        name=f"epoch_{epoch + 1}_finetune_ckpt.pt",
                        cleanup_after_logging=True,
                    )
