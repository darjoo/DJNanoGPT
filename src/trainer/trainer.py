...
from src.utils import get_memory_usage, get_system_info, load_checkpoint, save_checkpoint
...
class Trainer:
    """Trainer for pretraining GPT models from scratch.

    Handles the complete training loop including gradient accumulation,
    learning rate scheduling, checkpoint management, and metrics logging.
    Supports resuming from checkpoints and mixed precision training.
    """

    def __init__(self, model: GPT, training_config: TrainingConfig, device: str, resume_checkpoint: str | None = None):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to create checkpoint directory '{self.training_config.checkpoint_dir}': {e}"
            ) from e
    ...
    def init_optimizer(self):
        """Initialize the AdamW optimizer with configured hyperparameters."""
    ...
    def resume(self):
        ...
        checkpoint, state_dict = load_checkpoint(checkpoint_path, self.device)
    ...
    def get_lr(self, iteration):
        ...
        decay_ratio = (iteration - self.training_config.warmup_steps) / (
            self.training_config.lr_decay_iters - self.training_config.warmup_steps
        )
        assert 0.0 <= decay_ratio <= 1.0
        coefficient = 0.5 * (1.0 + math.cos(math.pi * decay_ratio))  # coeff ranges 0..1
        return self.training_config.min_lr + coefficient * (
            self.training_config.learning_rate - self.training_config.min_lr
        )
    ...
    def train_step(self):
        ...
        for _ in range(self.training_config.gradient_accumulation_steps):
            ...
            with torch.amp.autocast(device_type="cuda", enabled=(self.device == "cuda")):
                ...
                loss = (
                    loss / self.training_config.gradient_accumulation_steps
    ...
    def save_checkpoint(self, name: str = "ckpt.pt"):
        ...
        print(
            f"saving checkpoint to {ckpt_path}. Best val loss so far: "
            f"{self.best_val_loss:.4f} at iteration {self.iter_num}"
        )
    ...
    def evaluate(self):
        """Evaluate the model on train and validation sets and log metrics."""
    ...
    def train(self):
        ...
        try:
            while True:
                ...
                current_lr = (
                    self.get_lr(self.iter_num) if self.training_config.decay_lr else self.training_config.learning_rate
                )
